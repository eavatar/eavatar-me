...
class Console(object):
    ...
    def on_submit_btn_clicked(self):
        ...
        if self.script is None or len(self.script.strip()) == 0:
            self.append_message("No command given.")
            return

        try:
            self.validator.validate(self.script)
        except Exception as ex:
            if ex.message is not None and len(ex.message) > 0:
                reason = ex.message
            else:
                reason = repr(ex)
            self.append_message('Invalid command: %s' % reason)
            return
